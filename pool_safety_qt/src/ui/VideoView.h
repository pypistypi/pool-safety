...
private:
    ...
    /// Предел частоты показа.
    ///
    /// БЫЛО 40 МС (25 К/С) — РОВНО ЧАСТОТА КАМЕРЫ, ТО ЕСТЬ БЕЗ ОГРАНИЧЕНИЯ
    /// ВООБЩЕ. Показ каждого кадра — это перевод в QImage, масштабирование
    /// под размер клетки и перерисовка, и всё это идёт в главном потоке
    /// интерфейса — том же, что рисует меню, кнопки и реагирует на мышь.
    /// При 1920×1080 эта работа примерно вдвое тяжелее, чем при 1280×720, и
    /// если хотя бы один кадр обрабатывается дольше межкадрового промежутка,
    /// весь интерфейс на это время подвисает — оператор видит это как фриз.
    /// С четырьмя такими панелями на одном потоке заминки складываются.
    ///
    /// 80 мс (12,5 к/с) вдвое снижает эту нагрузку и оставляет заметный
    /// запас на медленный кадр, при этом для наблюдения за неподвижным по
    /// большей части бассейном движение остаётся гладким на глаз. Частота
    /// разбора (детектор, позы) от этого не меняется — она считается по
    /// отдельному, гораздо более редкому таймеру.
    static constexpr qint64 kMinIntervalMs = 80;
