...
core::AlarmCircumstance circumstanceFor(const core::DangerReport &report)
{
    ...
    case core::Situation::Splashing:
        // Та же опасность, что и «завис в воде» — ранняя стадия по Ф. Пиа
        // (см. SituationRules.h), только человек ещё двигается, а не застыл.
        return core::AlarmCircumstance::Drowning;
    ...
    connect(helpMenu->addAction(QStringLiteral("Что программа умеет и чего не умеет")),
        ...
        QMessageBox::information(this, QStringLiteral("О системе наблюдения"),
            QStringLiteral(
                ...
                "Она не ставит диагноз «человек тонет» — надёжной модели распознавания "
                "утопления не существует. Она замечает, что что-то не так, и зовёт "
                "оператора посмотреть.\n\n"
                "ВСЕ ШЕСТЬ ПОЛОЖЕНИЙ, КОТОРЫЕ ПРОГРАММА УМЕЕТ ЗАМЕЧАТЬ:\n\n"
                "Надёжно (положение и неподвижность измеряются точно):\n"
                "  • человек лежит и не двигается;\n"
                "  • человек упал и не поднялся — резкий переход в горизонталь, "
                "а не спокойное лежание.\n\n"
                "С оговорками — признак похож на беду, но не доказывает её:\n"
                "  • человек завис в воде вертикально и не продвигается — поздняя, "
                "тихая стадия утопления (инстинктивная реакция по Ф. Пиа, 1974): "
                "тело замирает, руки непроизвольно прижаты к воде, криков нет;\n"
                "  • резкие движения и брызги на месте, без продвижения — ранняя, "
                "энергичная стадия той же беды («водное бедствие» по той же работе): "
                "человек ещё барахтается, но не плывёт. От игры у бортика по одной "
                "позе не отличить, поэтому тревогу это положение поднимает только "
                "после проверки на объекте;\n"
                "  • ребёнок без взрослых рядом — по пропорциям тела (голова к "
                "туловищу), не проверено ни на одном настоящем ребёнке;\n"
                "  • неуверенная, шаткая походка рядом с водой — только подозрение, "
                "никогда не тревога: опьянение, головокружение и травму по видео "
                "не различить.\n\n"
                "Отдельно: в зоне есть люди и сколько их — это не тревога, а "
                "присутствие, показывается всегда.\n\n"
